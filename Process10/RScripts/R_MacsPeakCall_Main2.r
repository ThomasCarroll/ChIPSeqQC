...
SampleSheet2 <- RunMacsPeakCallerPipeline(SampleSheet,WkgDir=WkgDir,JobString,MaxJobs=75,PLs=PipeLineLocations,Config="Config")

#write.table(SampleSheet,"SampleSheet.csv",sep=",",row.names=F,quote=F)

write.table("Complete",file.path(PipeLineLocations@WorkFlowDir,paste(JobString,"_MacsPeakCall_Main_2.txt",sep="")),col.names=T,row.names=F,sep=",",quote=F)
