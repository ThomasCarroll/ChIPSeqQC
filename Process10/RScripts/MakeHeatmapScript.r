...
#NameOfFile="TempMe"
    ...
    if(toPlot == "Centre"){
          ...
          if(!is.null(NameOfFile)){
            ...
            system(paste("/lustre/mib-cri/carrol09/Work/MyPipe/matrix2png-1.2.2/matrix2png -data ",NameOfFile," -mincolor 255:250:250  -range 0:0.5 > ",paste(NameOfFile,"png",sep="."),sep=""),wait=T,intern=F)
...
Bigger3 <- cbind(as.vector(Biggertemp[,1]),Biggertemp[,2],Biggertemp[,3],paste(Biggertemp[,1],Biggertemp[,2],Biggertemp[,3],sep="_"),Biggertemp[,sortBy])
colnames(Bigger3) <- c("Peak_seqnames","Peak_start","Peak_end","PeakID",sortBy)
...
SomeColors <- c("red","green","blue","darkorange4")
...
png(OutAverage,width=2000,height=1000)
...
#Args[1] <-  "AR_SampleSheet.csv"
TempSampleSheet <- read.delim(Args[1],sep=",",h=T)
#Args[2] <-  "New_AR_EV.V.Gr3_HesOE.Bic.ls2_EV.Bic.ls2"
PicName <- Args[2]
#Args[3] <- "RPKM__Empty_Vehicle_AR_Vehicle"
sortBy <- Args[3]

DistanceToUse <- as.numeric(as.vector(Args[4]))
#Args[4] <- "New_AR_EV.V.Gr3_HesOE.Bic.ls2_EV.Bic.ls2.txt"
PeakSet <- read.delim(Args[5],sep="\t",header=T)




HeatMapAndProfiles(PeakSet,TempSampleSheet,DistanceToUse=DistanceToUse,PicName,sortBy)
