...
Histfiles <- dir(path=PLs@CovDir,pattern="*.hist",full.names=T)
forPic <- gsub("_Processed.hist","",dir(path=PLs@CovDir,pattern="*.hist"))
...
ggsave(P,filename=file.path(WkgDir,"HTML_Report","Plots","CoveragePlot.png"),type="cairo")
...
CoverageResults <- dir(path=PLs@FragLengthDir,pattern="*.FragCovLog",full.names=T)
...
ggsave(P,filename=file.path(WkgDir,"HTML_Report","Plots","CoverageShiftPlot.png"),type="cairo")
}
...
CoverageResults <- dir(path=PLs@FragLengthDir,pattern="*.CorrFragfull",full.names=T)
...
png(file.path(WkgDir,"HTML_Report","Plots","FragmentLengthPlot.png"),type="cairo")
...
png(file.path(WkgDir,"HTML_Report","Plots","SDsPlot.png"),type="cairo")
...
png(file.path(WkgDir,"HTML_Report","Plots","GinisPlot.png"),type="cairo")
...
png(file.path(WkgDir,"HTML_Report","Plots","AdjustedGinisPlot.png"),type="cairo")
...
png(file.path(WkgDir,"HTML_Report","Plots","AllCoverageSDGiniPlot.png"),width=2000,height=2000,type="cairo")
par(mfrow=c(3,1))
dotchart2(SDs,SampleNames,groups,col="darkred",dotsize=2,lcolor="orange",lty=2,xlab="SD of Coverage")
dotchart2(SDsOfInput, SampleNames, groups, add=TRUE,col="darkblue",dotsize=2,lcolor="orange",lty=2)
legend("topright",legend=c("Sample","Input_Used"),fill=c("darkred","darkblue"))
...
GCFiles <- dir(path=file.path(PeakDirectory,"PeakProfiles"),pattern="*GC.txt$",full.names=T)
CountFiles <- dir(path=file.path(PeakDirectory,"PeakProfiles"),pattern="*_Counts.bed$",full.names=T)
namesForGraph <- gsub("_GC.txt","",dir(path=file.path(PeakDirectory,"PeakProfiles"),pattern="*GC.txt$",full.names=F))
...
ggsave(p,file=file.path(WkgDir,"HTML_Report","Plots",paste(Caller,"GCcontentInpeaks.png",sep="_")),type="cairo")
}


GetCoverageProfilePlot <- function(SampleSheet){
ss <- SampleSheet
TSSProfilesFiles <- dir(PLs@CovDir,pattern="*Processed.RData",full.names=T)
...
ggsave(p,file=file.path(WkgDir,"HTML_Report","Plots","AverageTSSPlot.png"),width=20,height=8,type="cairo")
while(dev.cur() != 1){
dev.off(which = dev.cur())
}
...
TSSProfilesFiles <- dir(file.path(PeakDirectory,"PeakProfiles"),pattern="*_InGenePeakProfile.txt",full.names=T)
TSSProfilesFiles2 <- dir(file.path(PeakDirectory,"PeakProfiles"),pattern="*_InGenePeakProfile.txt",full.names=F)
...
#save(p,file=file.path(WkgDir,"Coverage","AverageTSSPlot.RData"))
ggsave(p,file=file.path(WkgDir,"HTML_Report","Plots",paste(Caller,"AveragePeakTSSPlot.png",sep="_")),width=20,height=8,type="cairo")
}
...
TSSProfilesFiles <- dir(file.path(PeakDirectory,"PeakProfiles"),pattern="*_InGenePeakCounts.txt",full.names=T)
TSSProfilesFiles2 <- dir(file.path(PeakDirectory,"PeakProfiles"),pattern="*_InGenePeakCounts.txt",full.names=F)
...
GenoCorr <- dir(file.path(PeakDirectory,"Between_Peaks"),pattern="*_GenoMetriCorr.txt",full.names=T)
GenoCorr2 <- dir(file.path(PeakDirectory,"Between_Peaks"),pattern="*_GenoMetriCorr.txt",full.names=F)
...
PosCountFiles <- dir(path=file.path(PeakDirectory,"PeakProfiles"),pattern="*CountsByPos.bed$",full.names=T)
NegCountFiles <- dir(path=file.path(PeakDirectory,"PeakProfiles"),pattern="*CountsByNeg.bed$",full.names=T)
namesForGraph <- gsub("_CountsByPos.bed","",dir(path=file.path(PeakDirectory,"PeakProfiles"),pattern="*CountsByPos.bed$",full.names=F))
...
ggsave(file=file.path(WkgDir,"HTML_Report","Plots",paste(Caller,"PosAndNegRatioCoverage.png",sep="_")),type="cairo")
...
OnHistFiles <- dir(path=file.path(PeakDirectory,"PeakProfiles"),pattern="*MergedCounts.bed$",full.names=T)
namesForGraph <- gsub("_MergedCounts.bed","",dir(path=file.path(PeakDirectory,"PeakProfiles"),pattern="*MergedCounts.bed$",full.names=F))
...
ggsave(file=file.path(WkgDir,"HTML_Report","Plots","OnAndOffRatioCoverage.png"),type="cairo")
cat(createGoogleGadget(MergedPicture),file=p)
}
...
OffHistFiles <- dir(path=file.path(PeakDirectory,"PeakProfiles"),pattern="*AllOutSidePeaks.hist$",full.names=T)
namesForGraph <- dir(path=file.path(PeakDirectory,"PeakProfiles"),pattern="*AllOutSidePeaks.hist$",full.names=F)
...
OnHistFiles <- dir(path=file.path(PeakDirectory,"PeakProfiles"),pattern="*AllInPeaks.hist$",full.names=T)
namesForGraph2 <- dir(path=file.path(PeakDirectory,"PeakProfiles"),pattern="*AllInPeaks.hist$",full.names=F)
...
ggsave(file=file.path(WkgDir,"HTML_Report","Plots",paste(Caller,"OnAndOffPeakCoverage.png",sep="_")),type="cairo")
}
...
BamLocations <-  file.path(PLs@BamDir,SampleSheet[,"Processed_bamFileName"])
...
GiniFiles <- dir(pattern="*_GiniCoverage.png",path=PLs@CovDir,full.names=F)
GiniFileLocations <- dir(pattern="*_GiniCoverage.png",path=PLs@CovDir,full.names=T)
...
if(length(grep("index.html",dir(path=file.path(PeakDirectory,"Motif",NewGroup[i],"Denovo")))) > 0){
ForSummaryDenovoMotifFile[i] <- file.path("..","Peaks",Caller,"Motif",NewGroup[i],"Denovo","index.html")
}
if(length(grep("ame.html",dir(path=file.path(PeakDirectory,"Motif",NewGroup[i],"Known")))) > 0){
ForSummaryKnownMotifFile[i] <- file.path("..","Peaks",Caller,"Motif",NewGroup[i],"Known","ame.html")
}
if(length(grep("dreme.html",dir(path=file.path(PeakDirectory,"Motif",NewGroup[i],"Denovo","dreme_out")))) > 0){
ForSummaryDremeMotifFile[i] <- file.path("..","Peaks",Caller,"Motif",NewGroup[i],"Denovo","index.html")
}
if(length(grep("meme.html",dir(path=file.path(PeakDirectory,"Motif",NewGroup[i],"Denovo","meme_out")))) > 0){
