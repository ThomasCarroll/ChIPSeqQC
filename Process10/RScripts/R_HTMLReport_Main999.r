...
SampleSheet <- RunReportingPipeline(SampleSheet,WkgDir=WkgDir,JobString,MaxJobs=75,PLs=PipeLineLocations,Config="Config")

#write.table(SampleSheet,"SampleSheet.csv",sep=",",row.names=F,quote=F)

write.table("Complete",file.path(PipeLineLocations@WorkFlowDir,paste(JobString,"_HTMLReport_Main_999.txt",sep="")),col.names=T,row.names=F,sep=",",quote=F)
