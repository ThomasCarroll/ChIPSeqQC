...
SampleSheet2 <- RunPeakProfilingPipeline(SampleSheet,WkgDir,JobString,MaxJobs=75,PipeLineLocations,"Config",Caller=PeakCaller)


#write.table(SampleSheet,"SampleSheet.csv",sep=",",row.names=F,quote=F)

write.table("Complete",file.path(PipeLineLocations@WorkFlowDir,paste(JobString,"_",PeakCaller,"_PeakProfile_Main_3.txt",sep="")),col.names=T,row.names=F,sep=",",quote=F)
