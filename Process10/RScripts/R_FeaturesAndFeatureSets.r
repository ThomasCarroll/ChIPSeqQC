...
GenesAndPromoters <- ExtendGenes(Genes,as.numeric(2000))


Peaks <- read.delim(Args[2],sep="\t",comment.char="#",header=F)
print(Peaks[1:10,])
#Peaks <- read.delim("jc1021Merged.bwa_Processed_peaks.bed",sep="\t",comment.char="#")
...
if(Args[6] == "Human_Internal"){
    ...
    GeneSets <- getGmt(Args[6])
    GOSymbols <- geneIds(GeneSets)
}




ResMatrix <- matrix(nrow=length(GOSymbols),ncol=6)
for(i in 1:length(GOSymbols)){
  ...
  ResMatrix[i,] <- c(names(GOSymbols)[i],LengthOfTerm,NoInTerm,Prior,NoInTerm/NoOfPeaks,Res)
}
...
FullMatrix <- ResMatrix
colnames(FullMatrix) <- c("Term_Name","Length_Of_Term","Number_Of_Peaks_In_Term","Expected_Proportion_Of_Peaks","Observed_Proportion_Of_Peaks","P_Value","Adjusted_P_Value")

}
FullMatrix <- FullMatrix[order(as.numeric(as.vector(FullMatrix[,"P_Value"])),decreasing=F),]
